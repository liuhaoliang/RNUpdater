...
@implementation RNUpdater

+ (RNUpdater *)sharedManager{
  static RNUpdater *sharedInstance = nil;
  static dispatch_once_t onceToken;
  dispatch_once(&onceToken, ^{
    sharedInstance = [[RNUpdater alloc] init];
  });
...
+ (BOOL)isValidJsBundleExist{
  ...
  return [target isEqualToString:self.appVersion];
}
...
+ (void)checkWithBridge:(RCTBridge*)bridge {
  ...
  dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(3 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
    ...
    NSDictionary* result = self.testData;
    manager.updateResult = result;
    BOOL isEqualNativeVersion = [result[@"target"] isEqualToString:self.appVersion];
